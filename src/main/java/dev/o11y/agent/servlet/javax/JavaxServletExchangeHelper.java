...
/** Entry/exit bridge for legacy {@code javax.servlet}; policy execution stays namespace-neutral. */
public final class JavaxServletExchangeHelper {
  ...
  private static final String STATE_ATTRIBUTE =
      JavaxServletExchangeHelper.class.getName() + ".state";
  ...
  private static final java.util.logging.Logger DIAGNOSTIC_LOGGER =
      java.util.logging.Logger.getLogger(JavaxServletExchangeHelper.class.getName());

  private JavaxServletExchangeHelper() {}

  public static State enter(Object request, Object response) {
    CallDepth depth = CallDepth.forClass(JavaxServletExchangeHelper.class);
    ...
    try {
      ...
      if (httpRequest.getAttribute(STATE_ATTRIBUTE) instanceof State) {
        return State.reentry();
      }
      return State.start(httpRequest, httpResponse);
    } catch (RuntimeException error) {
      DIAGNOSTIC_LOGGER.log(Level.FINE, "o11y_javax_servlet_capture=setup_skipped");
      return State.outerNoop();
    }
  }
    ...
    static State start(HttpServletRequest request, HttpServletResponse response) {
      ...
      State state =
          new State(
              ...
              candidate,
              selectedRequestHeaders(request, requestHeaderNames),
              responseHeaderNames,
              HttpBodyPolicyEngine.selectQueryParameters(request.getQueryString(), queryNames),
              context,
              new AtomicBoolean());
      ...
      try {
        ...
        if (outcome == CompletionOutcome.COMPLETED && originalRequest.isAsyncStarted()) {
          try {
            ...
            async.addListener(new JavaxServletExchangeAsyncListener(this));
            return;
        ...
        CallDepth.forClass(JavaxServletExchangeHelper.class).decrementAndGet();
      }
    }
